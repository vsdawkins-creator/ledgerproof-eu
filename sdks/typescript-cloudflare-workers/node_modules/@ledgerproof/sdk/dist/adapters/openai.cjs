...
function stringify(value) {
  ...
  if (typeof value === "object") {
    ...
    return "{" + keys.filter((k) => obj[k] !== void 0).map((k) => escapeString(k) + ":" + stringify(obj[k])).join(",") + "}";
  }
...
var Keypair = class _Keypair {
  ...
  static generate() {
    return new _Keypair(ed__namespace.utils.randomPrivateKey());
  }
  ...
  static fromHex(hex) {
    ...
    try {
      return new _Keypair(utils.hexToBytes(hex));
    } catch (exc) {
  ...
  sign(data) {
    return ed__namespace.sign(data, this.privateKey);
  }
  /** Verify a signature over `data` using this keypair's public key. */
  verify(signature, data) {
    try {
      return ed__namespace.verify(signature, data, this.publicKey());
    } catch {
      return false;
    }
  }
  ...
  publicKey() {
    return ed__namespace.getPublicKey(this.privateKey);
  }
  ...
  privateKeyHex() {
    return utils.bytesToHex(this.privateKey);
  }
  toString() {
...
var LedgerProof = class {
  ...
  buildPublishPayload(contentDict, contentType, sequence, prevHash) {
    ...
    const signatureHex = bytesToHex3(signature);
    return {
      publisher_id: this.publisherId,
      key_id: this.keyId,
      prev_hash: prevHash,
