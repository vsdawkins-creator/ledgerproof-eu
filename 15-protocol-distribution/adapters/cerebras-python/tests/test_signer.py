...
def test_canonical_cbor_deterministic():
    from ledgerproof_cerebras.canonical import canonical_cbor
    a = {"b": 2, "a": 1, "c": [1, 2, 3]}
    b = {"c": [1, 2, 3], "a": 1, "b": 2}
...
def test_verify_signature_helper_local_only():
    from ledgerproof_cerebras.signer import Ed25519Signer, verify_signature
    s = Ed25519Signer.generate()
    msg = b"local-verify"
    sig = s.sign(msg)
...
def test_signed_receipt_to_dict_has_fields():
    from ledgerproof_cerebras.canonical import canonical_cbor
    from ledgerproof_cerebras.signer import Ed25519Signer, SignedReceipt
    s = Ed25519Signer.generate()
