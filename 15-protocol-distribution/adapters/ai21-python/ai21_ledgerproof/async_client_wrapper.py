"""
Async client wrapper around `ai21.AsyncAI21Client`.

AI21 SDK (ai21>=3.0):
    from ai21 import AsyncAI21Client
    client = AsyncAI21Client(api_key=...)
    response = await client.chat.completions.create(model=..., messages=...)
    async for chunk in await client.chat.completions.create(..., stream=True):
        ...

Note: the AI21 async streaming surface returns an async iterable. We wrap it
with `_AsyncStreamIterator` and emit a single receipt on stream finalization.
"""

from __future__ import annotations

import logging
from typing import Any, AsyncIterator

from .canonical import IncrementalTextHasher
from .client_wrapper import (
    LedgerProofAI21,
    _default_reg_ctx,
    _extract_user_message_text,
    _stream_chunk_text,
)
from .emitter import Emitter, LogEmitter
from .schema import (
    JambaHybridAttestation,
    LongContextAttestation,
    RegulatoryContext,
    SchemaName,
)
from .signer import Ed25519Signer, Signer

logger = logging.getLogger(__name__)


class _AsyncStreamIterator:
    """Async equivalent of `_StreamIterator`."""

    def __init__(self, inner: AsyncIterator[Any], on_finish, hasher: IncrementalTextHasher):
        self._inner = inner
        self._on_finish = on_finish
        self._hasher = hasher
        self._last_chunk: Any = None
        self._finished = False

    def __aiter__(self) -> "_AsyncStreamIterator":
        return self

    async def __anext__(self) -> Any:
        try:
            chunk = await self._inner.__anext__()
        except StopAsyncIteration:
            await self._finalize()
            raise
        text = _stream_chunk_text(chunk)
        if text:
            self._hasher.update(text)
        self._last_chunk = chunk
        return chunk

    async def _finalize(self) -> None:
        if self._finished:
            return
        self._finished = True
        try:
            self._on_finish(self._last_chunk, self._hasher)
        except Exception as exc:  # noqa: BLE001
            logger.warning("LedgerProof async streaming receipt failed: %s", exc)

    async def aclose(self) -> None:
        await self._finalize()
        closer = getattr(self._inner, "aclose", None)
        if callable(closer):
            try:
                await closer()
            except Exception:  # noqa: BLE001
                pass


class _AsyncCompletionsProxy:
    def __init__(self, parent: "LedgerProofAsyncAI21", inner: Any):
        self._parent = parent
        self._inner = inner

    def __getattr__(self, name: str) -> Any:
        return getattr(self._inner, name)

    async def create(self, *args: Any, **kwargs: Any) -> Any:
        streaming = bool(kwargs.get("stream", False))
        user_text = _extract_user_message_text(kwargs.get("messages"))

        if streaming:
            hasher = IncrementalTextHasher()
            inner_iter = await self._inner.create(*args, **kwargs)

            def _on_finish(last_chunk: Any, h: IncrementalTextHasher) -> None:
                self._parent._emit_for_stream(
                    final_chunk=last_chunk,
                    user_message_text=user_text,
                    text_hasher=h,
                )

            return _AsyncStreamIterator(inner_iter, _on_finish, hasher)

        response = await self._inner.create(*args, **kwargs)
        try:
            self._parent._emit_for_response(
                response=response,
                user_message_text=user_text,
                streaming=False,
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning("LedgerProof receipt emission failed: %s", exc)
        return response


class _AsyncChatProxy:
    def __init__(self, parent: "LedgerProofAsyncAI21"):
        self._parent = parent
        self._inner = parent._inner.chat

    def __getattr__(self, name: str) -> Any:
        return getattr(self._inner, name)

    @property
    def completions(self) -> _AsyncCompletionsProxy:
        return _AsyncCompletionsProxy(self._parent, self._inner.completions)


class LedgerProofAsyncAI21(LedgerProofAI21):
    """
    Async drop-in wrapper for `ai21.AsyncAI21Client`.

    Inherits receipt construction from `LedgerProofAI21` but overrides
    `__init__` so we instantiate `AsyncAI21Client` rather than `AI21Client`,
    and `chat` so we expose the async proxy.
    """

    def __init__(
        self,
        deployer_id: str,
        *,
        client: Any | None = None,
        signer: Signer | None = None,
        emitter: Emitter | None = None,
        regulatory_context: RegulatoryContext | dict[str, Any] | None = None,
        schema: SchemaName = "chatbot_session/v1",
        long_context: LongContextAttestation | dict[str, Any] | None = None,
        jamba_hybrid: JambaHybridAttestation | dict[str, Any] | None = None,
        user_session_id: str | None = None,
        **ai21_kwargs: Any,
    ):
        self.deployer_id = deployer_id
        self.user_session_id = user_session_id
        if client is None:
            from ai21 import AsyncAI21Client  # lazy import
            client = AsyncAI21Client(**ai21_kwargs)
        self._inner = client
        self._signer: Signer = signer or Ed25519Signer()
        self._emitter: Emitter = emitter or LogEmitter()
        self._schema: SchemaName = schema
        if isinstance(regulatory_context, dict):
            regulatory_context = RegulatoryContext(**regulatory_context)
        self._reg_ctx: RegulatoryContext = regulatory_context or _default_reg_ctx()
        if isinstance(long_context, dict):
            long_context = LongContextAttestation(**long_context)
        self._long_context: LongContextAttestation | None = long_context
        if isinstance(jamba_hybrid, dict):
            jamba_hybrid = JambaHybridAttestation(**jamba_hybrid)
        self._jamba_hybrid: JambaHybridAttestation | None = jamba_hybrid

    @property
    def chat(self) -> _AsyncChatProxy:  # type: ignore[override]
        return _AsyncChatProxy(self)

    def __getattr__(self, name: str) -> Any:
        return getattr(self._inner, name)
