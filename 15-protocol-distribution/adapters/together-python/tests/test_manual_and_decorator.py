...
from ledgerproof_together.signer import Ed25519Signer


def _fake_response(text: str = "Hi from Llama.", model: str = "meta-llama/Llama-3.3-70B-Instruct-Turbo"):
...
def test_emit_receipt_open_model_inference_infers_when_omitted():
    captured: list = []
    signed = emit_receipt(
        response=_fake_response(model="Qwen/Qwen2.5-72B-Instruct-Turbo"),
        deployer_id="acme-eu",
...
def test_emit_receipt_explicit_open_model_wins():
    captured: list = []
    signed = emit_receipt(
        response=_fake_response(model="meta-llama/Llama-3.3-70B-Instruct-Turbo"),
        deployer_id="acme-eu",
        user_message_text="Hi",
        schema="open_model_inference/v1",
...
def test_emit_image_generation_receipt_hashes_image_bytes():
    captured: list = []
    signed = emit_image_generation_receipt(
        response=_fake_response(model="black-forest-labs/FLUX.1-schnell-Free"),
        deployer_id="acme-eu",
