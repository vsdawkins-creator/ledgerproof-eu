"""
Client wrapper tests using mocked Together clients (no network, no real API key).
"""
...
from ledgerproof_together.signer import Ed25519Signer, verify


# ---------------------------------------------------------------------------
# Fixtures: shape a Together ChatCompletionResponse-like object (OpenAI shape)
# ---------------------------------------------------------------------------
...
def test_sync_create_emits_receipt_and_returns_response_unchanged():
    ...
    assert signed["receipt"]["model"]["provider"] == "together"
    assert signed["receipt"]["model"]["model_id"] == "meta-llama/Llama-3.3-70B-Instruct-Turbo"
...
def _fake_stream_chunks(text_parts: list[str], model: str = "meta-llama/Llama-3.3-70B-Instruct-Turbo"):
    """Mimic Together ChatCompletionChunk stream (OpenAI-compatible shape)."""
    chunks = []
...
def test_async_create_emits_receipt():
    ...
    wrapper = LedgerProofAsyncTogether(
        deployer_id="acme-eu-test",
        client=fake_inner,
        emitter=QueueEmitter(captured.append),
        signer=Ed25519Signer(),
...
def test_open_model_inference_schema_uses_supplied_attribution():
    captured: list = []
    ...
    signed = captured[0]
    assert signed["receipt"]["schema"] == "open_model_inference/v1"
    assert signed["receipt"]["open_model"]["underlying_model_provider"] == "meta"
    assert signed["receipt"]["open_model"]["host_provider"] == "together"
    assert signed["receipt"]["open_model"]["model_license"] == "llama-3.3-community"


def test_open_model_inference_schema_infers_attribution_when_omitted():
    """If schema is open_model_inference/v1 and no attribution is given,
    the adapter should infer (family, provider) from the model_id."""
    captured: list = []
    wrapper = _make_wrapper(captured, schema="open_model_inference/v1")
    wrapper.chat.completions.create(
        model="meta-llama/Llama-3.3-70B-Instruct-Turbo",
        messages=[{"role": "user", "content": "Hi"}],
    )
...
def test_images_generate_emits_image_generation_receipt():
    captured: list = []
    fake_image_item = SimpleNamespace(url="https://together.example/image.png", b64_json=None)
    fake_response = SimpleNamespace(
        id="img_001",
        model="black-forest-labs/FLUX.1-schnell-Free",
        data=[fake_image_item],
    )
    fake_inner = MagicMock()
    fake_inner.images.generate.return_value = fake_response
    ...
    response = wrapper.images.generate(
        model="black-forest-labs/FLUX.1-schnell-Free",
    ...
    assert len(captured) == 1
    signed = captured[0]
    assert signed["receipt"]["schema"] == "image_generation/v1"
    assert signed["receipt"]["model"]["model_id"] == "black-forest-labs/FLUX.1-schnell-Free"
    roles = [c["role"] for c in signed["receipt"]["content_refs"]]
