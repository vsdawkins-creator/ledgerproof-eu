...
for example inside a custom orchestration layer or non-standard Mistral call shape.
"""
...
def emit_receipt(
    ...
    *,
    ...
    """
    Build, sign, and emit a receipt for a Mistral ChatCompletionResponse.
...
def _extract_assistant_text(response: Any) -> str | None:
    """
    Pull plain text out of a Mistral ChatCompletionResponse, defensively.

    Mistral response shape (mistralai>=1.0):
        response.choices[0].message.content  -> str or list[ContentChunk]
    """
    if response is None:
        return None
    ...
    if message is None:
        # Possibly a dict-style response
    ...
    if isinstance(content, list):
        # Mistral may return a list of content chunks; concatenate text parts.
        parts: list[str] = []
...
def extract_tool_uses(response: Any) -> list[ToolUseRef]:
    """
    Inspect a Mistral ChatCompletionResponse for function/tool calls and hash them.
    ...
    for call in tool_calls:
        tool_use_id = getattr(call, "id", None) or (call.get("id") if isinstance(call, dict) else None) or "unknown"
        ...
        # Arguments may already be a JSON string in Mistral SDK; normalise.
        if isinstance(arguments, str):
            try:
                arguments_obj = json.loads(arguments)
                input_bytes = canonical_encode(arguments_obj) if isinstance(arguments_obj, dict) else arguments.encode("utf-8")
