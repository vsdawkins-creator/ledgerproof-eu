...
logger = logging.getLogger("snowflake_cortex_ledgerproof.emitter")
...
class StdoutSink(ReceiptSink):
    def write(self, receipt: SignedReceipt) -> None:
        print(f"[ledgerproof-snowflake-cortex receipt] {json.dumps(receipt.to_dict())}")
...
class AsyncEmitter:
    ...
    The Cortex call returns to the caller before this emitter touches
    the sink.
    """
    ...
    def __init__(self, sink: ReceiptSink, *, max_queue: int = 1024):
        ...
        self._thread = threading.Thread(
            target=self._run, name="lpr-snowflake-cortex-emitter", daemon=True
        )
        self._thread.start()
