"""@lpr_track decorator for arbitrary Cortex-using functions.

C6: stream-aware — if the wrapped function returns a generator/iterator
(as in any streamed Cortex `Complete` path), the receipt is emitted after
the stream is fully consumed, over the concatenated output.
"""
    ...
    *,
    ...
    def deco(fn: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def _emit(result_text: str, latency_ms: float) -> None:
            ...
            if name == "GeneratedContentV1":
                fields["content_hash"] = _sha256(result_text)
            elif name == "EnterpriseDataLineageV1":
                fields["content_hash"] = _sha256(result_text)
            elif name == "ChatbotSessionV1":
                fields["completion_hash"] = _sha256(result_text)
            elif name == "CortexSearchRagV1":
                fields["completion_hash"] = _sha256(result_text)
                # A search_service_name is mandatory on this schema; the
                # caller must supply it via extra_fields.
            if extra_fields:
