...
def hash_bytes(data: bytes) -> bytes:
    """SHA-256 of raw bytes. Used for streaming chunks / opaque JSON payloads."""
    return hashlib.sha256(data).digest()


class IncrementalTextHasher:
    """
    Stream-aware SHA-256 hasher for streaming Bedrock responses (constraint C6).

    Bedrock streaming responses arrive as EventStream events with `chunk` payloads
    that decode to JSON containing model-specific delta fields. Feed text deltas
    as they're extracted; finalize once the stream completes.
    """
