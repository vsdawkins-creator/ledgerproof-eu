...
from ledgerproof_cohere.canonical import (
...
def test_incremental_hasher_handles_multibyte_utf8():
    """Cohere is heavily multilingual — make sure multibyte deltas hash correctly."""
    inc = IncrementalTextHasher()
    inc.update("Schöne ")
    inc.update("Grüße")
    digest = inc.digest()
    assert digest == hashlib.sha256("Schöne Grüße".encode("utf-8")).digest()
    assert inc.byte_count == len("Schöne Grüße".encode("utf-8"))
