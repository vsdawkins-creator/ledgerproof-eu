...
class IncrementalTextHasher:
    """
    Stream-aware SHA-256 hasher for streaming Cohere responses (constraint C6).

    Feed text deltas as they arrive (Cohere `chat_stream` yields `content-delta`
    events whose `.delta.message.content.text` is the increment); finalize once
