...
logger = logging.getLogger("ledgerproof_groq.emitter")
...
class StdoutSink(ReceiptSink):
    def write(self, receipt: SignedReceipt) -> None:
        print(f"[ledgerproof-groq receipt] {json.dumps(receipt.to_dict())}")
...
class AsyncEmitter:
    ...
    def __init__(self, sink: ReceiptSink, *, max_queue: int = 1024):
        ...
        self._thread = threading.Thread(
            target=self._run, name="lpr-groq-emitter", daemon=True
        )
        self._thread.start()
