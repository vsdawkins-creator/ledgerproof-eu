"""
`@lpr_track` decorator for user functions that wrap a Reka call.

Use when you already have a function that returns a Reka ChatResponse and you
want a receipt emitted on each invocation without restructuring your code to
...
def lpr_track(
    ...
    *,
    ...
    """
    Decorator factory. The wrapped function MUST return a Reka ChatResponse
    (or anything with `.id`, `.model`, and `.responses[0].message.content`).
