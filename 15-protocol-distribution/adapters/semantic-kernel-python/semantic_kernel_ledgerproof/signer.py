...
and `public_key_bytes() -> bytes`. Microsoft enterprise users will
typically pair this adapter with `AzureKeyVaultSigner` for the production
signer; that backend is intentionally stubbed (not implemented in MVP)
because key-management policy lives with the deployer.
"""
...
class BaseSigner(ABC):
    ...
    def public_key_b64(self) -> str:
        ...
        return (
            base64.urlsafe_b64encode(self.public_key_bytes())
            .rstrip(b"=")
            .decode("ascii")
        )
...
# --- Phase 4 HSM backend stubs (not implemented in MVP) ---------------------
#
# class AzureKeyVaultSigner(BaseSigner):
#     """Azure Key Vault Ed25519 signer (managed-identity auth).
#
#     Recommended production path for Microsoft enterprise Semantic Kernel
#     deployments. Implementation: azure-keyvault-keys + DefaultAzureCredential.
#     """
