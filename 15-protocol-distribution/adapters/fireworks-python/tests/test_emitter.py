...
from ledgerproof_fireworks.emitter import (
    LogEmitter,
    MultiEmitter,
    QueueEmitter,
    WebhookEmitter,
...
def test_multi_emitter_swallows_child_exceptions():
    captured = []
    ...
    em = MultiEmitter(_Boom(), QueueEmitter(captured.append))
    em.emit(_sample_receipt())
    assert captured  # second child still ran
