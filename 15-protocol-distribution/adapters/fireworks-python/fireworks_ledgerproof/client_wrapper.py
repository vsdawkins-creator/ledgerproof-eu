"""
Synchronous client wrapper around `fireworks.client.Fireworks`.
...
Image generation is exposed via a dedicated `image_wrapper.py` helper; the
Fireworks SDK exposes `client.image.generate(...)` (singular) for FLUX.

Fireworks SDK reference (fireworks-ai>=0.15) — OpenAI-compatible shape:
...
def _stream_chunk_text(chunk: Any) -> str:
    """
    Extract incremental text delta from a Fireworks streaming chunk.

    OpenAI-compatible shape (Fireworks returns the same):
...
class _StreamIterator:
    """
    Wraps Fireworks's streaming iterator. For each yielded chunk we update an
...
class _CompletionsProxy:
    ...
    def __init__(self, parent: "LedgerProofFireworks", inner: Any):
        self._parent = parent
        self._inner = inner
...
class _ChatProxy:
    ...
    def __init__(self, parent: "LedgerProofFireworks"):
        self._parent = parent
...
class _ImageProxy:
    """
    Wraps `client.image` (singular — Fireworks naming) and intercepts
    `.generate(...)` for FLUX models.
    """
    ...
    def generate(self, *args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            logger.warning("LedgerProof FLUX image-receipt emission failed: %s", exc)
        return response


class LedgerProofFireworks:
    """
    Drop-in wrapper for `fireworks.client.Fireworks`.

    Usage:
        from fireworks_ledgerproof import LedgerProofFireworks

        client = LedgerProofFireworks(deployer_id="acme-eu", api_key="fw_...")
    ...
    All other attributes/methods of the underlying Fireworks client are forwarded.
    """
    ...
    def __init__(
        ...
        **fireworks_kwargs: Any,
        ...
        if client is None:
            from fireworks.client import Fireworks  # lazy import — keeps test mocking simple
            client = Fireworks(**fireworks_kwargs)
        self._inner = client
    ...
    @property
    def image(self) -> _ImageProxy:
        """Fireworks uses singular `client.image` for FLUX endpoints."""
        return _ImageProxy(self)

    def __getattr__(self, name: str) -> Any:
        # Forward any unknown attribute (embeddings, audio, files, fine-tuning,
        # rerank, ...) to the underlying Fireworks client.
        return getattr(self._inner, name)
    ...
    def _resolve_open_model(self, model_id: str) -> OpenModelAttribution | None:
        ...
        if self._schema in ("open_model_hosted/v1", "flux_image_generation/v1"):
    ...
    def _emit_for_image(
        ...
        """Emit a flux_image_generation/v1 receipt for client.image.generate calls.

        Fireworks's `client.image.generate(...)` may return:
          - an object with `.image` (PIL Image) or `.image_bytes`
          - an object with `.data` (list of image entries, OpenAI-shaped)
          - raw bytes
        """
        import hashlib
        ...
        # Force flux_image_generation/v1 schema for this code path; keep deployer
        # attribution if they supplied one.
        prior_schema = self._schema
        try:
            self._schema = "flux_image_generation/v1"  # type: ignore[assignment]
            self._build_and_emit(
                response=response,
                content_refs=content_refs,
                tool_uses=[],
...
def _extract_image_bytes(response: Any) -> list[bytes]:
    """
    Best-effort extraction of image bytes from a Fireworks image response.

    Handles:
      - PIL.Image objects on `.image` (saves to PNG in memory)
    ...
    # 1. PIL.Image on .image
    pil_image = getattr(response, "image", None)
    ...
    if isinstance(image_bytes, (bytes, bytearray)):
        blobs.append(bytes(image_bytes))

    # 3. OpenAI-style .data[*].b64_json
    ...
    if data:
        for item in data:
            ...
            if isinstance(b64, str) and b64:
                try:
                    blobs.append(_b64.b64decode(b64))
                except Exception:  # noqa: BLE001
                    pass
