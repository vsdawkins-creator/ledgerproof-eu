...
or a non-standard Fireworks call shape.
"""
...
def emit_receipt(
    ...
    *,
    ...
    """
    Build, sign, and emit a receipt for a Fireworks ChatCompletion response
    ...
    # If the deployer chose open_model_hosted/v1 without supplying explicit
    # attribution, attempt best-effort inference from the model id.
    if schema == "open_model_hosted/v1" and open_model is None:
...
def _extract_assistant_text(response: Any) -> str | None:
    """
    Pull plain text out of a Fireworks ChatCompletion, defensively.

    Fireworks returns OpenAI-compatible shape:
...
def extract_tool_uses(response: Any) -> list[ToolUseRef]:
    """
    Inspect a Fireworks ChatCompletion for function/tool calls and hash them.

    Fireworks (OpenAI-compatible) shape:
        response.choices[0].message.tool_calls -> list of
            ToolCall(id=..., function=FunctionCall(name=..., arguments=...))

    Also handles FireFunction-style function_call (single object).
    """
    ...
    # FireFunction-style: single function_call object on the message.
    if not tool_calls:
        ...
        if fn_call is not None:
            tool_calls = [
                type("ToolCall", (), {
                    "id": "firefunction_call",
                    "function": fn_call,
                })()
            ] if not isinstance(fn_call, dict) else [{"id": "firefunction_call", "function": fn_call}]
...
# ---------------------------------------------------------------------------
# Image-generation helpers (FLUX on Fireworks)
# ---------------------------------------------------------------------------


def emit_flux_image_generation_receipt(
    response: Any,
    deployer_id: str,
    *,
    prompt_text: str,
    ...
    """
    Emit a flux_image_generation/v1 receipt for Fireworks image endpoints
    (FLUX schnell / dev / pro, etc.).

    Fireworks `client.image.generate(...)` typically returns an object with
    image bytes (PIL Image / bytes). Pass either:
    ...
    model_ref = _build_model_ref(response)
    if model_ref.model_id == "unknown" and model_id:
        model_ref = model_ref.model_copy(update={"model_id": model_id})
