...
that lets verifiers re-derive the signed digest offline.
...
def canonical_encode(obj: Mapping[str, Any]) -> bytes:
    ...
    Raises:
        TypeError: if `obj` contains non-CBOR-encodable values.
    """
    if not isinstance(obj, Mapping):
        raise TypeError(f"canonical_encode requires a Mapping, got {type(obj).__name__}")
    return cbor2.dumps(
        dict(obj),
        canonical=True,
...
def receipt_digest(obj: Mapping[str, Any]) -> bytes:
    ...
    This is the bytestring that the Ed25519 signer signs.
    """
