...
Receipts NEVER ride inside the Gemini response payload. They go to one of:
  - LogEmitter — structured stdout/stderr/logger
  - StderrEmitter — convenience: log to stderr
  - WebhookEmitter — HTTPS POST to deployer-controlled endpoint
  - QueueEmitter — generic callable interface (SQS/Pub/Sub/Kafka/Redis)
...
logger = logging.getLogger("ledgerproof_google_ai.emitter")
