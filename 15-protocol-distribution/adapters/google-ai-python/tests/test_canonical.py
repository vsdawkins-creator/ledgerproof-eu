...
from ledgerproof_google_ai.canonical import (
...
def test_incremental_hasher_matches_full_hash():
    ...
    assert inc.byte_count == len("Hello, Gemini!".encode("utf-8"))
...
def test_canonical_nested_maps_are_deterministic():
    a = {"outer": {"b": 2, "a": 1}, "list": [1, 2, 3]}
    b = {"list": [1, 2, 3], "outer": {"a": 1, "b": 2}}
    assert canonical_encode(a) == canonical_encode(b)
