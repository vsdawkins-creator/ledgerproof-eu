...
MVP uses an ephemeral in-process key. Production deployments will plug
HSM-backed signers (AWS KMS, GCP KMS, YubiHSM, Azure Key Vault) via the same
`Signer` protocol.
"""
...
class AzureKeyVaultEd25519Signer:
    """Stub for Azure Key Vault Ed25519 signing. Not implemented in MVP."""

    def __init__(self, key_identifier: str):
        self.key_identifier = key_identifier

    def sign(self, message: bytes) -> bytes:  # pragma: no cover
        raise NotImplementedError("Azure Key Vault signer not implemented in MVP")
